...
public:
    ...
    // Index of dest node
    int dest;
    // Weight of edge. Default is 1
    int weight;
    //
    int flow;
    GraphEdge() : src(0), dest(0), weight(1) {
        this->index = GraphEdge::counter++;
    }
    GraphEdge(int src, int dest) : src(src), dest(dest), weight(1) {
        this->index = GraphEdge::counter++;
    }
...
public:
    ...
    // Matrix containing all edges
    map<int, map<int, GraphEdge>> edgesMatrix; 
    Graph(){}
    ...
    void insertEdge(GraphEdge edge){
            ...
            // Add edge to matrix
            this->edgesMatrix[edge.src][edge.dest] = edge;
    }
    ...
    GraphEdge* getEdge(int u, int v){
        // If edge is not found
        if(this->edgesMatrix.find(u) == this->edgesMatrix.end() && this->edgesMatrix[u].find(v) == this->edgesMatrix[u].end()){
            return new GraphEdge(u, v);
        }
        return &(this->edgesMatrix[u][v]);
    }
